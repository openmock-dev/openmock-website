...
- **Standalone server**: a runtime whose whole job is serving documents
  ...
  resolution of §2 *and* the admin API of §3, which is the CLI's
...
| Bind ports; resolve them by §2's order; fail loudly, never silently reassign | n/a (binds no sockets) | **MUST** | **MUST** |
| Surface the resolved name → port topology (discovery) | n/a | **MUST**, via its own UI or API | **MUST**, via the admin API (§3) |
| Expose the HTTP admin API on 4400 (§3) | n/a | not expected | **MUST** |
